...
def test_comressor_fp16_manual():
    for device in ["cpu", "cuda"]:
        ...
        compressed_M = sparse_spmv.compress(M)
...
def test_spmv_manual():
    ...
    compressed_M = sparse_spmv.compress(SAMPLE_MATRIX.cuda())
    V_cuda = torch.rand((SAMPLE_MATRIX_COLS,), device="cuda", dtype=torch.float16)
    expected_result = M_cuda @ V_cuda
    result = sparse_spmv.multiply(compressed_M, V_cuda)
...
def run_test(M_rows, M_cols, density, seed):
    ...
    compressed_M = sparse_spmv.compress(M_cuda)
    result = sparse_spmv.multiply(compressed_M, V_cuda)
    expected_result = M_cuda @ V_cuda
...
def test_compile():
    ...
    f = lambda x: sparse_spmv.multiply(compressed_M, x + 1) - 1
